...
def clean_go_enrichment(file, direction=None, clean_terms=True, FDR_filter=None):
    ...
    if clean_terms:
        ...
        enrichment.replace(
            {"term description": "Antigen processing.*"},
            "Antigen processing",
            regex=True,
            inplace=True,
        )
    ...
    enrichment = enrichment.assign(
        FDR=lambda x: x["false discovery rate"],
...
def combine_go_enrichments(
    version_,
    type_,
    dir_="../results/string_data/",
    include_c1=False,
    directed=False,
    top_n=None,
    **kwargs
    ...
    d = {
        "C2": clean_go_enrichment(
            dir_ + version_ + "/g2" + ".enrichment." + type_ + ".tsv",
            direction=directions["C2"],
            **kwargs
        ),
        "C3": clean_go_enrichment(
            dir_ + version_ + "/g3" + ".enrichment." + type_ + ".tsv",
            direction=directions["C3"],
            **kwargs
        ),
    }
    ...
    if include_c1:
        d.update(
            {
                "C1": clean_go_enrichment(
                    dir_ + version_ + "/g1" + ".enrichment." + type_ + ".tsv",
                    direction=directions["C1"],
                    **kwargs
                )
            }
        )
    ...
    df = (
        pd.concat(d)
        .reset_index(0)
        .rename({"level_0": "C"}, axis=1)
        .assign(
            FDR_rank=lambda x: x.groupby(["C", "direction"])["neglogFDR"].rank(
                method="first", ascending=False
            )
        )
        .assign(
            enrichment_rank=lambda x: x.groupby(["C", "direction"])["enrichment"].rank(
                method="first", ascending=False
            )
        )
    )
    ...
    if top_n is not None:
        df = (
            df.assign(
                rank=lambda x: x.groupby(["C", "direction"])["neglogFDR"].rank(
                    method="first", ascending=False
                )
            )
...
def get_cell_genes(
    ...
    if which == "wen":
        ...
        cell_genes = (
            ...
            .replace(
                {"label": {"purple": "Neurons", "brown": "Synapses", "blue": "Glia"}}
            )
            .set_index("label")
            .loc[["Neurons", "Synapses", "Glia"], :]
            .reset_index()
        )
    elif which == "zeng":
        ...
        if include == "only_lake":
            ...
            cell_genes = cell_genes.loc[
                lambda x: (x["Paper"] == "Lake") | (~x["Class"].str.contains("Neuro")),
                :,
            ]
        ...
        if subtype:
            cell_genes = cell_genes.assign(
                Class=lambda x: [
                    t if "Neuro" in c else c for c, t in zip(x["Class"], x["Type"])
                ]
            )
        ...
        if combine_layers:
            cell_genes = cell_genes.assign(
                Class=lambda x: [
                    re.sub("(a|b|c|d|e)", "", c) if bool(re.search("(Ex|In)", c)) else c
                    for c in x["Class"]
                ]
            )

        if combine_ex_in:
            cell_genes = cell_genes.replace(
                {"Class": {"Neuro-Ex": "Neuro", "Neuro-In": "Neuro"}}
            )
        else:
        ...
        cell_genes = (
            cell_genes.melt(
                id_vars=["Type", "Paper", "Cluster", "Class"], value_name="gene"
            )
...
def get_synapse_genes():
    synapse_genes = (
        pd.read_csv("../data/synaptome_all.csv", usecols=["gene_symbol"])
        .dropna()
        .iloc[:1886]
    )
...
def get_cell_genes_weighted(which=None, normalize=True):
    ...
    def clean_lake(df, normalize=True):
        df = (
            df.rename({"cluster": "label", "Gene": "gene"}, axis=1)
            .melt(id_vars=["label", "gene"], var_name="which", value_name="weight")
            .loc[lambda x: x["label"] == x["which"]]
            .drop("which", axis=1)
            .dropna()
        )
        ...
        if normalize:
            df = (
                ...
                .assign(
                    weight=lambda x: x.groupby("label").transform(
                        lambda y: fisher_norm(y)
                    )
                )
                .fillna(1)
                .reset_index()
...
def get_layer_genes(which="maynard", add_hse_genes=False):
    he_layers = (
        pd.read_csv("../data/he_layers.csv")
        .loc[
            :,
            ["Gene symbol", "Layer marker in human", "Log2FC to other layers in human"],
        ]
    ...
    maynard_tstat = (
        ...
        .set_index("gene")
        .set_axis(["L1", "L2", "L3", "L4", "L5", "L6", "WM"], axis=1)
        .reset_index()
...
def get_intelligence_gwas_genes():
    lee2018 = (
        pd.read_csv(
            "../data/gwas/lee2018_tableS7.csv", header=1, usecols=["Gene symbol"]
        )
        .dropna()
        .rename({"Gene symbol": "gene"}, axis=1)
        .assign(label="lee2018")
    )
    ...
    ## UPDATE THIS
    gwas_intelligence = (
        pd.read_csv("../data/gwas/intelligence.csv", header=0)
        .drop("sniekers2017", axis=1)
        .melt(var_name="label", value_name="gene")
        .dropna()
    )
    ...
    gwas_intelligence = (
        ...
        gwas_intelligence.replace({"label": rename_dict}).assign(
            label=lambda x: pd.Categorical(
                x["label"], ordered=True, categories=rename_dict.values()
            )
        )
    )
